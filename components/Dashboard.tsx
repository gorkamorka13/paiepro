                                ...
                                return (
                                    <tr key={payslip.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                                        ...
                                        <td className="px-4 py-4 whitespace-nowrap">
                                            ...
                                            ) : (payslip.siretNumber || payslip.urssafNumber || (payslip.cesuNumber && payslip.cesuNumber.startsWith('Z'))) ? (
                                                <div className="text-xs text-gray-500 mt-1">
                                                    {payslip.siretNumber && !(payslip.cesuNumber?.startsWith('Z') && (!payslip.urssafNumber || payslip.urssafNumber.length <= 2)) && (
                                                        <div className="flex items-center gap-1">
                                                            <span className="font-semibold text-[10px] text-gray-400 uppercase">SIRET</span>
                                                            {payslip.siretNumber}
                                                        </div>
                                                    )}
                                                    {payslip.urssafNumber && !(payslip.cesuNumber?.startsWith('Z') && (!payslip.siretNumber || payslip.urssafNumber.length <= 2)) && (
                                                        <div className="flex items-center gap-1">
                                                            <span className="font-semibold text-[10px] text-gray-400 uppercase">URSSAF</span>
                                                            {payslip.urssafNumber}
                                                        </div>
                                                    )}
                                                    {((payslip.cesuNumber?.startsWith('Z') && (!payslip.siretNumber || !payslip.urssafNumber || payslip.urssafNumber.length <= 2)) || (!(payslip.siretNumber || payslip.urssafNumber) && payslip.cesuNumber)) && (
                                                        <div className="flex items-center gap-1">
                                                            <span className="font-semibold text-[10px] text-gray-400 uppercase">CESU</span>
                                                            {payslip.cesuNumber}
                                                        </div>
                                                    )}
                                                </div>
