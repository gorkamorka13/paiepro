...
import {
  analyzeDocumentHybrid,
  extractDataTraditional,
...
export async function processPayslipAction(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `❌ Validation des données extraites échouée pour ${file.name}:`,
        validationErr,
      );
      throw new Error(
        `Données extraites invalides : ${validationErr instanceof Error ? validationErr.message : "Détails inconnus"}`,
      );
    }
    ...
    if (duplicate) {
      console.warn(
        `⚠️ Doublon détecté après analyse pour ${file.name} (correspond à ID: ${duplicate.id}).`,
      );
...
export async function getPayslipsAction(): Promise<ActionResult<Payslip[]>> {
  try {
    ...
    console.log(
      `📡 [getPayslipsAction] ${payslips.length} bulletins récupérés.`,
    );
  ...
  try {
    ...
    if (payslip.fileUrl) {
      try {
        ...
        console.error(
          `⚠️ Erreur suppression blob (${payslip.fileName}):`,
          blobError,
        );
...
export async function getUsageStatsAction() {
  try {
    ...
    const totalTokens =
      (logStats._sum?.inputTokens || 0) + (logStats._sum?.outputTokens || 0);
...
export async function deleteMultiplePayslipsAction(
  ids: string[],
  ...
  try {
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Impossible de supprimer les bulletins",
    };
  }
}
...
export async function reanalyzePayslipAction(
  ...
  try {
    ...
    if (method === "ai") {
      extractedData = await analyzeDocument(
        existingPayslip.fileUrl,
        fileMetadata,
        context,
      );
    } else {
      const result = await extractDataTraditional(
        existingPayslip.fileUrl,
        fileMetadata,
        context,
      );
      if (!result) {
        const errorMessage =
          "L'extraction traditionnelle a échoué (données insuffisantes). Consultez les logs pour plus de détails.";
        await prisma.payslip.update({
          where: { id },
          data: {
            processingStatus: "failed",
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Erreur lors de la ré-analyse";
    await prisma.payslip.update({
      where: { id },
      data: {
        processingStatus: "failed",
